...
def get_dependencies(fp, metadata):
    ...
    dependency_metadata = dict(zip([
            ...
            'model',
            'time_horizon'],
        metadata['depends'].split('.')))

    dependency_metadata['team'] = 'climate'
...
def get_metadata(fp):
    ...
    metadata['note'] = ''.join(metadata['note'].split('\n'))
    metadata['method'] = ''.join(metadata['method'].split('\n'))
    
    metadata['variable'] = '/'.join(metadata['variable'].split('.'))
    metadata['depends'] = '/'.join(metadata['depends'].split('.'))

    del metadata['version']
    del metadata['depends_var']
    del metadata['command']


    # Get metadata from filename components
    fname_components = [
        'geography',
        'ncdc',
        'frequency',
